...
Grab the [jar](build/yajtl.jar) from the `build/` folder and go to it. Check out the examples in `examples/`. You will also need a copy of [Java OSC](https://github.com/hoijui/JavaOSC). I have also included [one](lib/javaosc.jar) in the `lib/` folder.
